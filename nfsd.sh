...
if [ -z "${SHARED_DIRECTORY}" ]; then
  ...
  echo "{{SHARED_DIRECTORY}} {{PERMITTED}}({{READ_ONLY}},fsid=0,{{SYNC}},no_subtree_check,no_auth_nlm,insecure,root_squash)" >> /etc/exports
...
if [ ! -z "${SHARED_DIRECTORY_2}" ]; then
  ...
  echo "{{SHARED_DIRECTORY_2}} {{PERMITTED}}({{READ_ONLY}},{{SYNC}},no_subtree_check,no_auth_nlm,insecure,root_squash)" >> /etc/exports
